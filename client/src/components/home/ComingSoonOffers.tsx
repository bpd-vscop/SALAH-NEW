...
const MAX_COMING_SOON_PRODUCTS = 24;
...
export const ComingSoonOffers: React.FC = () => {
    ...
    const loadComingSoon = async () => {
      try {
        const response = await productsApi.list({
          tags: ['coming soon'],
          sort: 'newest',
          limit: MAX_COMING_SOON_PRODUCTS,
        });

        const list = (response.products ?? []).slice(0, MAX_COMING_SOON_PRODUCTS);

        if (isMounted) {
          setProducts(list);
        }
      } catch (error) {
        console.error('Failed to load coming soon products', error);
      } finally {
        if (isMounted) {
          setLoading(false);
        }
    ...
    <section className="mb-8 w-[88%] mx-auto py-6">
      <div className="mb-6 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            to="/products?tags=coming%20soon"
