...
const ensureEstimateNumber = async () => {
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const candidate = `EST${Date.now()}${Math.floor(Math.random() * 90 + 10)}`;
    // eslint-disable-next-line no-await-in-loop
    const exists = await Estimate.exists({ estimateNumber: candidate });
    if (!exists) {
      return candidate;
    }
  }
  throw badRequest('Unable to generate estimate number');
};
...
const listEstimates = async (req, res, next) => {
  try {
    ...
    res.json({ estimates: estimates.map((estimate) => estimate.toJSON()) });
  } catch (error) {
    next(error);
  }
};
...
const getEstimate = async (req, res, next) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      throw notFound('Estimate not found');
    }
    const estimate = await Estimate.findById(id);
    if (!estimate) {
      throw notFound('Estimate not found');
    }
    res.json({ estimate: estimate.toJSON() });
  } catch (error) {
    next(error);
  }
};
...
const createEstimate = async (req, res, next) => {
  try {
    ...
    if (!itemsInput.length) {
      throw badRequest('Estimate items are required');
    }
    ...
    const items = itemsInput.map((item) => {
      ...
      if (productId && !mongoose.Types.ObjectId.isValid(productId)) {
        throw badRequest('Invalid product identifier on estimate item');
      }
      return {
    ...
    const estimateNumber = await ensureEstimateNumber();
    ...
    res.status(201).json({ estimate: estimate.toJSON() });
  } catch (error) {
    next(error);
  }
};
...
const updateEstimateStatus = async (req, res, next) => {
  try {
    ...
    if (!['pending', 'completed', 'canceled'].includes(status)) {
      throw badRequest('Invalid estimate status');
    }
